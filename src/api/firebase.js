...
export async function getCart(userId) {
  ...
  export async function getReviews(productId) {
    return get(ref(database, `reviews/${productId}`))
    .then((snapshot) => {
      if(snapshot.exists()) {
        return Object.values(snapshot.val()); 
      }
  ...
  export async function addOrUpdateToReview(user, productId, text) {
    const newReview = {userId: user.uid, image: user.photoURL, name: user.displayName, review: text}
    return set(ref(database, `reviews/${productId}/${user.uid}`), newReview); 
  }
  
  export async function removeFromReview(productId, userId) {
